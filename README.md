...
The AVL Answer: An AVL tree balances itself. It checks the balance on its own after every insertion and does rotations to keep the tree "short" and "flat." This makes sure that search, insert, and delete operations always take O(log n) time, even in the worst case.2. Efficiency Over Alternatives
...
o	If smaller, move to the Left child; if larger, move to the Right.
o	Repeat until an empty spot (NULL) is found and insert the new Book node.
...
o	Left Rotation: Used when the right side is too heavy.
o	Right Rotation: Used when the left side is too heavy.
